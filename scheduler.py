...
class EnDeDiffWarmup(LRScheduler):
    """
    Applies a warm up period to the learning rate.
    """
    ...
    def __init__(self, optimizer, warmup, total_steps, last_epoch=-1):
        self.warmup = warmup
        self.total_steps = total_steps
        super(EnDeDiffWarmup, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        progress = self.last_epoch / self.total_steps

        output = []
        for nb,base_lr in enumerate(self.base_lrs):
            warmup = self.warmup[nb]
            if progress < warmup:
                output.append(base_lr * progress / warmup)
            else:
                output.append(base_lr * max((progress - 1.0) / (warmup - 1.0), 0.))
        return output
