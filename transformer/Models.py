...
def get_non_pad_mask(seq):
    assert seq.dim() == 2
    return seq.ne(PAD).type(torch.float).unsqueeze(-1)  # b_size * seq_len * 1
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, dropout, embeddings, operatorNums):
        super().__init__()
        ...
        # self.pos_embeddings = nn.Embedding(30, d_model, padding_idx=0)

        self.conversion = nn.Linear(self.embeddings.embedding_dim, d_model)
        ...
        # self.operator_num = torch.tensor([2754,1637,8,118,800,3894]).type(self.pos_emb.type())
    ...
    def forward(
        ...
        if memory_mask is not None:
            ...
            src_pad_mask = memory_mask.expand(src_batch, tgt_len, src_len).type(torch.int32)
        else:
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout):
        ...
        self.conversion = nn.Linear(d_model * 3, d_model)
    ...
    def forward(
        ...
        """
        dec_mask = torch.gt(
            tgt_pad_mask + self.mask[:, : tgt_pad_mask.size(1), : tgt_pad_mask.size(1)].type(torch.bool), 0)

        input_norm = self.layer_norm_1(inputs)
        all_input = input_norm
        if previous_input is not None:
            all_input = torch.cat((previous_input, input_norm), dim=1).contiguous()
            dec_mask = None
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
            ...
            if type == "self":
                ...
                if layer_cache is not None:
                    ...
                    if layer_cache["self_keys"] is not None:
                        key = torch.cat((layer_cache["self_keys"].to(device), key), dim=2).contiguous()
                    if layer_cache["self_values"] is not None:
                        value = torch.cat((layer_cache["self_values"].to(device), value), dim=2).contiguous()
        ...
        if predefined_graph_1 is not None:
            ...
            attn = torch.cat([attn[:, :-1], attn_masked.unsqueeze(1)], 1).contiguous()
